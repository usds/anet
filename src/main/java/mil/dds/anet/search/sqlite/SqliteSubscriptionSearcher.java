...
public class SqliteSubscriptionSearcher extends AbstractSearcherBase
  ...
  public AnetBeanList<Subscription> runSearch(SubscriptionSearchQuery query, Person user) {
    ...
    final StringBuilder sql =
        new StringBuilder("/* getSubscriptionsForPosition */ SELECT * FROM \"subscriptions\" "
            + "WHERE \"subscriptions\".\"subscriberUuid\" = :positionUuid "
            + "ORDER BY subscriptions.\"updatedAt\" DESC");
    sql.append(" LIMIT :limit OFFSET :offset)");
    final AnetBeanList<Subscription> result =
        new AnetBeanList<>(query.getPageNum(), query.getPageSize(), new ArrayList<Subscription>());

    final Query q = getDbHandle().createQuery(sql.toString()).bindMap(args)
        .bind("offset", query.getPageSize() * query.getPageNum())
        .bind("limit", query.getPageSize());
    final List<Subscription> list = q.map(new SubscriptionMapper()).list();
    result.setList(list);
